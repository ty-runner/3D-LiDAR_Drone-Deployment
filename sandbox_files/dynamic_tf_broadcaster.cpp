...
public:
  ...
  {
    // Existing params (pan/tilt/lidar)
    ...
    pan_to_tilt_z_ = this->declare_parameter<double>("pan_to_tilt_z", 0.0);

    tilt_to_lidar_x_ = this->declare_parameter<double>("tilt_to_lidar_x", 0.00);
    tilt_to_lidar_y_ = this->declare_parameter<double>("tilt_to_lidar_y", 0.0);
    tilt_to_lidar_z_ = this->declare_parameter<double>("tilt_to_lidar_z", 0.00);

    double pub_rate = this->declare_parameter<double>("publish_rate", 50.0);

    // New MAVROS/pose-to-tf params
    mavros_topic_ = this->declare_parameter<std::string>("mavros_pose_topic", "/mavros/local_position/pose");
    // By default we publish map -> base_link, but make it configurable:
    mavros_parent_frame_ = this->declare_parameter<std::string>("mavros_parent_frame", "map");
    mavros_child_frame_  = this->declare_parameter<std::string>("mavros_child_frame", "base_link");
    // If true, use the incoming message's header.frame_id as the transform's parent frame
    use_msg_header_frame_ = this->declare_parameter<bool>("mavros_use_msg_header_frame", false);
    ...
    // MAVROS pose subscription
    pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
      mavros_topic_, 10,
      std::bind(&DynamicTFBroadcaster::mavros_pose_cb, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(get_logger(), "Subscribed to MAVROS pose topic: %s", mavros_topic_.c_str());
    RCLCPP_INFO(get_logger(), "MAVROS pose -> TF: %s -> %s",
                mavros_parent_frame_.c_str(), mavros_child_frame_.c_str());
  }

private:
  // servo angles callback (unchanged)
  ...
  // MAVROS pose callback -> broadcast a transform
  void mavros_pose_cb(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
  {
    geometry_msgs::msg::TransformStamped t;
    // Use msg header stamp so transform is time-aligned
    t.header.stamp = msg->header.stamp;

    // Choose parent frame: either the msg header.frame_id or configured param
    if (use_msg_header_frame_ && !msg->header.frame_id.empty()) {
      t.header.frame_id = msg->header.frame_id;
    } else {
      t.header.frame_id = mavros_parent_frame_;
    }
    t.child_frame_id = mavros_child_frame_;

    // Straight mapping from Pose -> Transform
    ...
    // If you want throttled logging for debugging:
    RCLCPP_DEBUG(get_logger(), "Publishing MAVROS TF: %s -> %s (t=%.3f)",
  ...
  // periodic publishing for the pan/tilt frames (unchanged)
  void publish_tf()
  {
  ...
  std::string mavros_parent_frame_;
  std::string mavros_child_frame_;
  bool use_msg_header_frame_;
};
